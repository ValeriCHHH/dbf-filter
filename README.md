...
* 🌐 **Поддержка кириллицы:** корректно обрабатывает исходные кодировки `CP866`, `Windows-1251` и сохраняет CSV в `UTF-8*
* ⚡ **Эффективность:** поток чтения обрабатывает большие таблицы с минимальным потреблением оперативной памяти.
...
```bash
dbf-filter -i <входной_файл.dbf> -o <выходной_файл.csv> [ДОП_ПАРАМЕТРЫ]
```
### Доступные параметры
